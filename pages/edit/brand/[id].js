...
const EditBrandPage = () => {
  ...
  const onGetBrand = useCallback(async () => {
    if(!isInitialized) return false
    const RealBrands = Moralis.Object.extend('RealBrands');
    const query = new Moralis.Query(RealBrands);
  ...
  const onUpdateBrand = async () => {
    ...
    const RealBrands = new Moralis.Query('RealBrands');
    RealBrands.equalTo('objectId', brandId);
    const object = await RealBrands.first();
    await object.save()
      .then((x) => {
        x.set('title', title);
