...
const getAvatarGroup = (members) => {
  ...
  const onClickChatUser = async (member) => {
    const response = await Moralis.Cloud.run('GetUserById', { userId: member.user_id });
    const userAccounts = response.data.attributes.accounts;
    router.push(`/sales/${userAccounts[0]}`, undefined, { shallow: true });
  }


  if (members.length === 1) {
    return (
      <div className='messaging__channel-header__avatars' onClick={() => onClickChatUser(members[0])}>
